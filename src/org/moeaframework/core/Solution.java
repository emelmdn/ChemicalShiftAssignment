...
import org.moeaframework.problem.chemicalShiftAssignment.Atom;
...
public class Solution implements Serializable {
	...
	private double G = 0; //Emel added this variable to store the value of G, aggregated score function result in FLYA
	...
	//Emel added
	//will create flya.prot structured file for the given solution in the path with the name a0runCounter_solutionCounter.prot
	public void writeResultsToFile(String rootResultPath, int runCounter, int tuningCounter, int solutionCounter) throws IOException{
		int i;
		double d;
		double dec = Math.pow(10, 3);  //how many digits should be shown after period for double 
		String resultPathOriginal;
		
		File dir = new File(String.valueOf(rootResultPath));
		
		if(!dir.exists()){
             dir.mkdir();
         }
		
		rootResultPath += "/"+(runCounter+1)+".Run/";
		resultPathOriginal = rootResultPath;
		
		File directory = new File(String.valueOf(rootResultPath));
		if(!directory.exists()){
             directory.mkdir();
         }
		
		dir = new File(String.valueOf(rootResultPath));
		if(!dir.exists()){
             dir.mkdir();
         }
		rootResultPath += "/"+(tuningCounter+1)+ "/";
		

		directory = new File(String.valueOf(rootResultPath));
		if(!directory.exists()){
             directory.mkdir();
         }
		String fileName = rootResultPath +"a0"+(tuningCounter+1)+ "_"+(solutionCounter+1)+".prot"; //first file: a01_1
		
		try{	
			PrintWriter writer = new PrintWriter(fileName, "UTF-8");
				
			for (i = 0; i < this.atoms.size(); i++){
				d = Math.round((this.atoms.get(i).getAverageFrequency())*dec)/dec;
				writer.print(String.format("%-10d %-10s %-10s %-10s %-10s \n", (i+1), d, "0.000",this.atoms.get(i).getGenericAtomName(),this.atoms.get(i).getResidueNumber()));
				//same line but right aligned (minuses removed):
				//writer.print(String.format("%10d %10s %10s %10s %10s \n", (i+1), d, "0.000",this.atoms.get(i).getGenericAtomName(),this.atoms.get(i).getResidueNumber()));
			}
			writer.close();
			System.out.println("Atom frequencies are written to the file: "+ fileName);
		}  finally {
			
			
		}
	
		dir = new File(String.valueOf(resultPathOriginal));
		if(!dir.exists()){
             dir.mkdir();
         }
		resultPathOriginal += "/AllTogether/";
		
		directory = new File(String.valueOf(resultPathOriginal));
		
		if(!directory.exists()){
             directory.mkdir();
         }
		fileName = resultPathOriginal +"a0"+(tuningCounter+1)+ "_"+(solutionCounter+1)+".prot"; //first file: a01_1
